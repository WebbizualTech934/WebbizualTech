...
function updateSlider() {
    ...
    slideCount = Math.ceil(visibleItems.length / itemsPerSlide);
    ...
    if (visibleItems[currentIndex]) visibleItems[currentIndex].classList.add('active');

    const itemWidth = visibleItems[0]?.offsetWidth || 0;
    const gap = 30; // adjust if you use CSS gap
...
window.addEventListener('resize', () => {
    // Recalculate layout and dots when resizing
    updateSlider();
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateTestimonialSlider() {
        ...
        const itemWidth = testimonialItems[0].getBoundingClientRect().width + 20; // include gap
        const translateX = testimonialIndex * itemWidth;
        testimonialTrack.style.transform = `translateX(-${translateX}px)`;
    }

    testimonialNext.addEventListener('click', () => {
        const itemsPerSlide = window.innerWidth > 768 ? 2 : 1;
        const totalSlides = Math.ceil(testimonialItems.length / itemsPerSlide);
